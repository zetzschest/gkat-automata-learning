module Table :
  sig
    ...
    type output = SA.t
    type table = { top_rows : output M.t M.t; bottom_rows : output M.t M.t; }
    val top_row_indices : table -> S.t
    val bottom_row_indices : table -> S.t
    val column_indices : table -> S.t
    val construct_new_row :
      word -> S.t -> atom list -> (word -> atom -> bool) -> output M.t
    val word_times_alphabet : word -> alphabet -> word list
    type return_closed = None | Upper_Index of word
    val exists_equivalent_upper_row : output M.t -> table -> return_closed
    val add_to_top_rows :
      word ->
      output M.t ->
      table ->
      alphabet -> atom list -> (word -> atom -> bool) -> S.t -> S.t -> table
    val word_to_string : word -> int -> string
    val output_to_string : output -> int -> string
    ...
    val close :
      ...
      alphabet ->
      atom list ->
      (word -> atom -> bool) -> int -> table * transition_structure
    type output_structure = SA.t M.t
    val construct_output_structure : table -> output_structure
    val suffixes_of : word -> word list
    val add_counterexample :
      table -> word -> (word -> atom -> bool) -> atom list -> table
    val instantiate_table :
      (word -> atom -> bool) -> atom list -> alphabet -> table
    val automaton_from_table_structure :
      transition_structure ->
      output_structure -> (word option, int, int) Automata.Automata.kat_autom
    val lstar_moore :
      (word -> atom -> bool) ->
      ((word option, int, int) Automata.Automata.kat_autom -> bisimul_return) ->
      int -> int -> (word option, int, int) Automata.Automata.kat_autom * int
  end
